...
| `get_drug_competitors(name)` | `CompetitorRawData` (TypedDict) — returns raw `diseases: dict[str, set[str]]` (disease name to competitor drug names) and `drug_indications: list[str]` (approved indication names). Filters out diseases whose name contains any `BROADENING_BLOCKLIST` word (word-level match). Does **not** perform LLM dedup, sorting, or top-N slicing -- that logic lives in `RetrievalService.get_drug_competitors()`, which calls this method, merges duplicates via an LLM call, sorts by competitor count, and returns the top 15. |
